...
def isochrone_sim(grid,y_n_bins,x_n_bins):
	n_stars_act=sum(grid)
	grid_probs=grid/n_stars_act
	grid_means=n_stars*grid_probs

	for i in range(int(y_n_bins-1)):
		for j in range(int(x_n_bins-1)):
			grid_sim[i,j]=np.random.poisson(grid_means[i,j],1)

	hdu=fits.PrimaryHDU(grid)
	hdulist=fits.HDUList([hdu])
	hdulist.writeto('q26.fits')
...
grid_sim=0.0*np.empty([y_n_bins,x_n_bins])
...
isochrone_sim(grid,y_n_bins,x_n_bins)
